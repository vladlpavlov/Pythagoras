...
class NaN_Inducer(PFeatureMaker):
    ...
    def is_fitted_(self) -> bool:
        return ( hasattr(self, "transformabe_columns_")
                 and self.transformabe_columns_ is not None )
...
class Deduper(PFeatureMaker):
    ...
    def is_fitted(self) -> bool:
        return (hasattr(self, "columns_to_keep_")
                and bool(len(self.columns_to_keep_)) )
...
class NumericImputer(PFeatureMaker):
    ...
    def is_fitted(self) -> bool:
        return (hasattr(self,"fill_values_")
                and self.fill_values_ is not None)
    ...
    def transform(self
        ...
        X_num = X.select_dtypes(include="number")[self.input_columns()]
...
class NumericFuncTransformer(PFeatureMaker):
    ...
    def __init__(self
        ...
        super().__init__(parent_logger_name = parent_logger_name
                         ,new_logging_level = new_logging_level)
        self.positive_arg_num_functions = deepcopy(positive_arg_num_functions)
        self.any_arg_num_functions = deepcopy(any_arg_num_functions)

    def is_fitted(self):
        result = (hasattr(self, "columns_to_a_transform_")
                    and hasattr(self, "columns_to_p_transform_")
                    and self.columns_to_a_transform_ is not None
